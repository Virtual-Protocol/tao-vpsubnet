...
def inference(opt):
    ...
    if opt.use_cached_features:
        ...
        for dir in dir_list:
            ...
            juke_file_list = sorted(
                glob.glob(f"{dir}/*.npy"), key=stringintkey)
            ...
            if opt.out_length > 0:
                ...
                # use all files to process entire audio instead of random sample of output length definition
                rand_idx = 0
                file_list = file_list
                juke_file_list = juke_file_list
    ...
    for i in range(len(all_cond)):
        ...
        model.render_sample(
            data_tuple, "output_", opt.render_dir, render_count=-1, fk_out=fk_out, render=not opt.no_render
        )
